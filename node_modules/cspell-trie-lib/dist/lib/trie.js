...
class Trie {
    ...
    find(text, minCompoundLength = false) {
        ...
        return find_1.findWordNode(this.root, text, options).node;
    }
    /**
     *
    ...
    findCompound(text, minCompoundLength = defaultLegacyMinCompoundLength) {
        ...
        const r = find_1.findLegacyCompound(this.root, text, options);
        return r.node;
    }
    findExact(text) {
    ...
    has(word, minLegacyCompoundLength) {
        ...
        if (minLegacyCompoundLength) {
            ...
            const findOptions = find_1.createFindOptions({ legacyMinCompoundLength: len });
            return !!find_1.findLegacyCompound(this.root, word, findOptions).found;
        }
        return false;
    }
    /**
    ...
    hasWord(word, caseSensitive) {
        ...
        const f = find_1.findWord(this.root, word, findOptions);
    ...
    createFindOptions(options = {}) {
        const { caseInsensitivePrefix = this._options.stripCaseAndAccentsPrefix, compoundFix = this._options.compoundCharacter, forbidPrefix = this._options.forbiddenWordPrefix, } = options;
        const findOptions = find_1.createFindOptions({
            ...options,
