...
const cspellCosmiconfig = {
    ...
    loaders: {
        '.json': (_filename, content) => json.parse(content),
    },
};
const cspellConfigExplorer = cosmiconfig_1.cosmiconfig('cspell', cspellCosmiconfig);
const cspellConfigExplorerSync = cosmiconfig_1.cosmiconfigSync('cspell', cspellCosmiconfig);
...
function importSettings(fileRef, defaultValues = defaultSettings) {
    ...
    const settings = { ...defaultValues, id, ...readConfig(importRef) };
...
async function normalizeSearchForConfigResult(filename, searchResult) {
    var _a;
    let result;
    try {
    ...
    filename = (_a = result === null || result === void 0 ? void 0 : result.filepath) !== null && _a !== void 0 ? _a : filename;
    ...
    const id = [path.basename(path.dirname(filename)), path.basename(filename)].join('/');
    const finalizeSettings = { id, __importRef: importRef };
    const settings = { id, ...result.config };
    const pathToSettings = path.dirname(filename);
    Object.assign(finalizeSettings, normalizeSettings(settings, pathToSettings));
    return finalizeSettings;
}
function searchForConfig(searchFrom) {
