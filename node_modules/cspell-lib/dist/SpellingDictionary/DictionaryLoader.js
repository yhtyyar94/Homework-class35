...
async function refreshEntry(entry, maxAge, now) {
    if (now - entry.ts >= maxAge) {
        ...
        entry.ts = now;
        const pStat = fs_extra_1.stat(entry.uri).catch((e) => e);
...
function loadEntry(uri, options, now = Date.now()) {
    const dictionary = load(uri, options).catch((e) => createSpellingDictionary_1.createFailedToLoadDictionary(determineName(uri, options), uri, 'error', [e]));
    return {
        uri,
        options,
        ts: now,
        state: fs_extra_1.stat(uri).catch((e) => e),
        dictionary,
    };
}
...
async function loadSimpleWordList(filename, options) {
    ...
    return createSpellingDictionary_1.createSpellingDictionary(lines, determineName(filename, options), filename, options);
}
async function loadTrie(filename, options) {
    const lines = await fileReader_1.readLines(filename);
    return SpellingDictionaryFromTrie_1.createSpellingDictionaryTrie(lines, determineName(filename, options), filename, options);
}
